...
namespace AAMT
{
    ...
    public class SettingManager : ScriptableObject
    {
        ...
        public AssetSetting.BuildTarget editorRunTarget = AssetSetting.BuildTarget.editor;
        ...
        private static void LoadAssetSetting(AssetSetting.BuildTarget? buildTarget = null)
        {
            ...
            if (buildTarget != null) _instance.editorRunTarget = buildTarget.Value;
            if (_instance != null)
            {
                switch (_instance.editorRunTarget)
                {
...
#else
            var path =
                $"{Application.streamingAssetsPath}/{AssetSetting.PlatformToBuildTarget()}/{AAMTDefine.AAMT_BUNDLE_NAME}"
                    .ToLower();

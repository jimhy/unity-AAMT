...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    public SimpleAnimationPlayable()
    {
        ...
        playingStates = new List<StateInfo>();
    }

    public void AddToPlayingState(StateInfo state)
    {
        if(state.wrapMode != WrapMode.Loop)
        {
            if (!playingStates.Contains(state))
            {
                playingStates.Add(state);
            }
        }
    }

    public void CheckPlayingState(StateInfo state)
    {
        if (state.isDone && state.wrapMode != WrapMode.Loop)
        {
            if (playingStates.Contains(state))
            {
                onAnimationComplete?.Invoke(state.stateName);
                playingStates.Remove(state);
            }
        }
    }
    ...
    public void PlayAction(string name)
    {
        ...
        if (state.playableDuration > 0)
        {
            Crossfade(name,state.playableDuration);
        }
        else
        {
            Play(name);

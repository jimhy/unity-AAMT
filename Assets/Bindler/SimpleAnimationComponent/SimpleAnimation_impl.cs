...
{
    const string kDefaultStateName = "";
    ...
    [SerializeField]
    protected AnimatorCullingMode m_CullingMode = AnimatorCullingMode.AlwaysAnimate;
    ...
    private Dictionary<string, float> durationDic = new Dictionary<string, float>();
    
    public Action<string> OnAnimationComplete;

    public Action<string> onHitCallBackFunc;
    ...
    {
        onHitCallBackFunc?.Invoke(s);
    }

    private void OnSound(string content)
    {
        if (content == "") return;
        string[] idTable = content.Split(',');
        System.Random random = new System.Random();
        int index = random.Next(0, idTable.Length);
        uint soundId = uint.Parse(idTable[index]);
    }
        ...
        if (m_States != null)
        {
            foreach (var state in m_States)
            {
                ...
                if (state.clip)
                {
                    ...
                    durationDic.Add(state.name,state.duration);
                }
            }
        }

	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata_base v)
			{
				...
				o.rim = 1 - max(0, dot(worldViewDir, worldNormal));
				o.matcap = viewnormal.xyz * 0.5 + 0.5;
				return o;
			}
			...
			{
			{
				...
				fixed4 matcapDiffuse = tex2D(_MatCapDiffuse, i.matcap) * 4;
				fixed4 specTex = maincol.a;

				fixed4 matcap = tex2D(_CubeLightMap, i.matcap);
				fixed4 reflec = texCUBE(_Environ, i.reflectionDir) * _Specular * specTex;

				//fixed4 rim = pow(matcap,_RimRange);	
				fixed4 rim = _RimColor * pow(float4(i.rim,1), _RimRange) * _CapIntensity * matcap;
				
#if _FLOWON_ON
				fixed4 lightTexA = tex2D(_Flow,i.uv.zw).r * _Power * specTex * _FlowColor;
#else
				fixed4 lightTexA = 0;
#endif
				half4 col = maincol * matcapDiffuse + reflec + rim + lightTexA;
